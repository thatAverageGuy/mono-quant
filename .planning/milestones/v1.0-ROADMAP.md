...
**Key Decisions:**
- Model-agnostic design - Users have varied model sources, don't want to lock into HF ecosystem
- Weights-only approach - Simpler API, fewer dependencies, focused responsibility
- CLI + Python API - CI/CD needs automation, humans need convenience
- Build-phase only - Separation of concerns - quantization is build-time optimization
- Local imports pattern - Used local imports in helper functions to break circular import between core.quantizers and modules.linear

**Issues Resolved:**
- Fixed circular import between core.quantizers and modules.linear by moving imports to local scope
- Model-agnostic input handling verified to work with custom, HF-like, and pretrained models
