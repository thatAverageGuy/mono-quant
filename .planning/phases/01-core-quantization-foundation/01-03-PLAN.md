...
must_haves:
  truths:
    ...
    - "Bias is preserved when quantizing Linear layers"
  artifacts:
    ...
    - path: "src/mono_quant/modules/linear.py"
      provides: "QuantizedLinear module"
...
- FP16 approach is Claude's discretion (simple casting chosen)
...
4. Create quantize_conv2d_module() function (similar pattern):
   - Input: nn.Conv2d module
   - Quantize weight per-channel (axis=0 for output channels)
   - Return new Conv2d with quantized weights (or placeholder if full quantization not implemented)
